<?php
   ...
   if ($params == "0") {
      $sql = mysqli_query($con, "SELECT CONCAT(u.nombres, ' ', u.apellido_pat, ' ', u.apellido_mat) nombreDocente, v.id_vale AS id_vale, l.nombre_lab, v.fecha, v.hora_entrada, v.hora_salida, v.plantel, v.firma_docente, esp.carrera, v.id_estado_vale FROM vale AS v INNER JOIN usuario AS u ON u.id_usuario = v.id_nomina INNER JOIN personal AS p ON p.id_usuario = v.id_nomina INNER JOIN usuario AS us ON us.id_usuario = v.id_usuario INNER JOIN laboratorio AS l ON l.id_laboratorio = v.id_laboratorio INNER JOIN especialidad AS a ON a.id_division = v.id_division INNER JOIN estado_vale AS ev ON v.id_estado_vale = ev.id_estado_vale INNER JOIN especialidad AS esp ON esp.id_division = v.id_division WHERE v.id_estado_vale = 1 AND v.id_tipo_vale = 4 ");
      ...
      WHERE v.id_estado_vale = 1 AND v.id_tipo_vale = 1");
